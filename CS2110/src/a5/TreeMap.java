...
public class TreeMap {
    ...
    public static Wrapper2 getSplit(List<Node> b, int m, int n) {
        ...
        if(m>n) return null;
        double[] weight = new double[n-m+1];
        int h = m + (n-m)/2;
        for(int i = 0; i <= n-m; i++){
        	if(i==0)
        		weight[i] = b.get(i+m).weight;
        	else
        		weight[i] = weight[i-1] + b.get(i+m).weight;
        }
        if(weight[n-m]<=0){
        	return new Wrapper2(h, 0.5);
        }
        for(int i = 0; i < n - m; i++ ){
        	if(weight[i]>= SPLIT_RATIO * weight[n-m]){
        		h=i+m;
        		break;
        	}
        }
        return new Wrapper2(h, weight[h-m]/weight[n-m]);
    }
    ...
    private void sliceAndDice(List<Node> b, int m, int n,
        ...
        BoundingBox boxLeft = new BoundingBox(bbox);
        BoundingBox boxright = new BoundingBox(bbox);
        if(n==m){
        	b.get(m).block = new Block(bbox,new Color(0, 0, 127));
        	return;
        }
        Wrapper2 wp2 = getSplit(b, m, n);
//        if(w>h){
//        	boxLeft.high.x = boxLeft.low.x + wp2.d*(boxLeft.high.x-boxLeft.low.x);
//        	boxright.low.x = boxright.low.x + wp2.d*(boxright.high.x-boxright.low.x);
//        	sliceAndDice(b, m, wp2.k, boxLeft, (int) (w*wp2.d), h);
//        	sliceAndDice(b, wp2.k+1, n, boxright, (int) (w-w*wp2.d),h);
//        }
//        else{
//        	boxLeft.high.y = boxLeft.low.y + wp2.d*(boxLeft.high.y-boxLeft.low.y);
//        	boxright.low.y = boxright.low.y + wp2.d*(boxright.high.y-boxright.low.y);
//        	sliceAndDice(b, m,wp2.k, boxLeft, w, (int) (h*wp2.d));
//        	sliceAndDice(b, wp2.k+1, n, boxright, w,(int)(h-h*wp2.d));
//        }
        if((bbox.high.x-bbox.low.x)>(bbox.high.y-bbox.low.y)){
        	boxLeft.high.x = boxLeft.low.x + wp2.d*(boxLeft.high.x-boxLeft.low.x);
        	boxright.low.x = boxright.low.x + wp2.d*(boxright.high.x-boxright.low.x);
        	sliceAndDice(b, m, wp2.k, boxLeft, w, h);
        	sliceAndDice(b, wp2.k+1, n, boxright, w,h);
        }
        else{
        	boxLeft.high.y = boxLeft.low.y + wp2.d*(boxLeft.high.y-boxLeft.low.y);
        	boxright.low.y = boxright.low.y + wp2.d*(boxright.high.y-boxright.low.y);
