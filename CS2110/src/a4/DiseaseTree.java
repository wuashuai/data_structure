...
/* NetId(s): zw377, nnnnn. Time spent: hh hours, mm minutes.
...
public class DiseaseTree {
    ...
    public DiseaseTree add(Person p, Person c) throws IllegalArgumentException {
    	...
    	if(!contains(p)) throw new IllegalArgumentException("p is not in the DiseaseTree");
    	if(contains(c)) throw new IllegalArgumentException("c is already in this DiseaseTree");
    	
    	DiseaseTree dTree = new DiseaseTree(c);
    	getTree(p).children.add(dTree);
    ...
    public int widthAtDepth(int d) throws IllegalArgumentException {
        //TODO 4
    	int width=0;
        if(d<0) throw new IllegalArgumentException("depth < 0");
        if(d==0){
        	return 1;
        }
        if(children.isEmpty()){
        ...
        if(d>0){
        	d--;
        	for(DiseaseTree a : children){
        		width += a.widthAtDepth(d);
        	}
        };
        return width;
    }
    ...
    public Person getSharedAncestorOf(Person child1, Person child2) {
        //TODO 6
    	if(child1 == null || child2 == null||!contains(child2)||!contains(child1)) return null;
    	if(child1.equals(child2)) return child1;
    	if(root == child1 || root == child2) return root;
        if(children.contains(child2) && children.contains(child1))
        	return root;
    	for(DiseaseTree dt : children){
            if((dt.contains(child1) && !dt.contains(child2))||(dt.contains(child2) &&!dt.contains(child1))){
            	return root;
            }
            if (dt.getSharedAncestorOf(child1, child2)!=null) {
            	return dt.getSharedAncestorOf(child1, child2);
			}
    	}
        return null;
    }
    ...
    public boolean equals(Object ob){
    	...
    	if( ob instanceof DiseaseTree ){
    		if( root==null || ob==null ){
    			return false;
    		}
    		if(root.equals(((DiseaseTree) ob).root)){
    			if(children==null && (((DiseaseTree) ob).children)==null ){
    				return true;
    			}
    			for(DiseaseTree obt : ((DiseaseTree) ob).children){
    				for(DiseaseTree dt : children){
    					if(dt.equals(obt)) 
    						return true;
    				}
