...
public class GUI extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
    	...
    	FileTreeMap v = (FileTreeMap) treeView.getTreeMap();
    	int d = v.getMaxDepth();
    	if(e.getSource().equals(depthPlus)){
    		recomputeTreeMap(d+1);
    		return;
    	}
    	if(e.getSource().equals(depthMinus)){
    		if(d<=1)
    			return;
    		recomputeTreeMap(d-1);
    	}
    ...
    public class MouseEvents extends MouseInputAdapter {
        ...
        public @Override void mouseClicked(MouseEvent e) {
        	...
        	PointD v = new PointD(((double) e.getX())/treeView.getWidth(),((double) e.getY())/treeView.getHeight());
        	FileTreeMap treeMap = (FileTreeMap) treeView.getTreeMap();
        	TreeMap.Node root = treeMap.getRoot();
        	TreeMap.Node n = treeMap.getNodeContaining(root,v);
        	File file = treeMap.getFile(n);	
